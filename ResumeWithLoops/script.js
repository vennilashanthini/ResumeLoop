...
for(let i=0;i<myResume.length;i++){
    console.log("array",myResume[i]);
}

const objkeys = Object.keys(myResume);
const objvalues = Object.values(myResume);
console.log(objkeys);
console.log(objvalues);

for(let i =0;i<objkeys.length;i++){
    console.log("objkeys",objkeys[i]);
    console.log("objvalues",myResume[`${objkeys[i]}`]);
    console.log("objvalues",objvalues[i]);
}

//foreach ---> suitable for array
myResume.forEach((value,index,accArr)=>{
    console.log(
        `value:${value}
        index:${index}
        accArr:${accArr}`
...
//forin --> suitable for objects
for(key in myResume){
    console.log(
        `object key :${key}
         object value:${myResume[key]}
        `
    );
}
...
for(idx in myResume){
    console.log(
        `array idx :${idx}
         array value:${myResume[idx]}
        `
    );
}

//Forof  -->suitable for string
const samplestr = "Vennilashanthini"
